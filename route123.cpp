...
bool jumpOut = 0;
void dfs2(int now_pos, int cnt,
          int sum, int Target,
          int &ans, int &ans_path_num,
          int &tmp_path_num, int ans_path[],
          int tmp_path[], int &FLAG,
          bool visit[], bool must[])
{
	time_end_dfs2 = clock();
...
void updata_reSide()
{
	...
	///    priority_queue<node_detail> que;
//void SPFA_dp(int start, int des, int pre[], node_detail must_node[], int &nmust_node, bool must[], bool visit[]) {
//	nmust_node = 0;
//	int id[NODENUM_SIZE];   //记录顶点v在must_node中的下标
//	for (int i = 0;i<nodeNum;i++) {
//		Dist[i].dist = INF, Dist[i].id = i;
...
std::pair<std::vector<int>, int> dpe12solve(int s, int t ,bool *subset,int sizesub) {
	sizesub--;
    //printf("rlglllll\n");
    ...
    int x, aim, y, p, q, minans=0,w,cannum=0,cutnum,cutsubnum;
    Fe12q[s][0] = 1;
    Fe12c[s][0] = 0;
    ...
    while (!dpe12qu.empty()) {
        ...
        //printf("%d %d %d %d %d\n",x,y,Fe12[x][y],++Q,minans);
        dpe12qu.pop();
        //printf("%d %d %d\n",x,y,Fe12[x][y]);
        Fe12q[x][y] = 0;
//        if (Fe12[x][y]==sizesub) printf("%d %d\n",x,y);
        if (Fe12[x][y] == sizesub-1 || Fe12[x][y] == sizesub-2 || Fe12[x][y] == sizesub-4 || Fe12[x][y] == sizesub-8){
            you_dfs(x,t,);
        }
        if (x == t) {
            //if (Fe12[x][y] == sizesub-1) puts("asd---------------------------------------------");
            if (Fe12[x][y] == sizesub) {
            //	puts("asd");
...
void search_route(char *topo[MAX_EDGE_NUM], int edge_num, char *demand[MAX_DEMAND_NUM], int demand_num)
{
	...
	else
	{
		...
		ant = dpe12solve(Start_one,Target_one,must_one,must_pass_one.size());
		if(ant.first.size() == 0)
		{
			printf("hhhhhhhhh\n");
			FLAG_ONE = 0;
		...
		ant2 = dpe12solve(Start_two,Target_two,must_two,must_pass_two.size());
		if(ant2.first.size() == 0)
		{
...
*/
		...
		ant_bc1 = dpe12solve(Start_one,Target_one,must_one,must_pass_one.size());
		if(ant_bc1.first.size() == 0)
		{
